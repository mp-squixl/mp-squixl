...
# -----------------------------------------------------------------
# additions for using fonts created with font_to_py and written to
# the mp framebuffer using the CWriter module.
...
# Function to write text to screen via CWriter
def wtext(wbuf, font, text, x, y, fg_colour, bg_colour):
...
# ------------------------------------------------------------------------------
# a new class to create text labels to which of font can be assigned and will use the
# wtext function to wite the label to the framebuffer.

class UILabelWriter(UIControl):
    """A text label using imported fonts and the writer class."""
    def __init__(self, x, y, w, h, title="", callback=None,
                 fg_color=0xFFFF, bg_color=0x0000, text_color=0xFFFF, font=None):
        super().__init__(x, y, w, h, title, callback, fg_color, bg_color, text_color)
        self.font = font

    def draw(self, buf):
        if self.manager:
            self.bg_color=self.manager.current_screen.bg_color
        wtext(buf,self.font,self.title, self.x, self.y, self.text_color, self.bg_color)

    def set_text(self, text):
        # draw existing text as background colour
        if self.manager:
            temp_text_color=self.manager.current_screen.bg_color
            wtext(self.manager.buf,self.font,self.title, self.x, self.y, temp_text_color, self.bg_color)
...
class UIButton(UIControl):
    ...
    # --------------------------------------------------------------------
    # Note - this class has been altered to include a font parameter and to use
    # the wtext funtion in place of the buf.text to write the text to the framebuffer
    # ---------------------------------------------------------------------
    def __init__(self, x, y, w, h, title="", callback=None,
                 fg_color=0xFFFF, bg_color=0x0000, text_color=0xFFFF,font=None):
        super().__init__(x, y, w, h, title, callback, fg_color, bg_color, text_color)
    ...
    def draw(self, buf):
        ...
        wtext(buf,self.font,self.title, tx, ty, textcol, self.bg_color)
...
class UICheckBox(UIControl):
    ...
    def draw(self, buf):
        ...
        wtext(buf,self.font,self.title, self.x + self.w + 6, ly, self.text_color, self.bg_color)
